"""
Agent Service SDK - 基于 Claude Agent SDK 的 Agent 服务
...
支持两种模式：
1. claude-agent-sdk 模式（推荐）- 使用官方 Agent SDK
2. anthropic 模式（备选）- 直接使用 Anthropic SDK
"""
...
# 尝试导入 claude-agent-sdk，如果失败则使用 anthropic
try:
    from claude_agent_sdk import ClaudeSDKClient, ClaudeAgentOptions, tool, create_sdk_mcp_server
    AGENT_SDK_AVAILABLE = True
    print("[AgentSDKService] 使用 claude-agent-sdk 模式")
except ImportError:
    AGENT_SDK_AVAILABLE = False
    print("[AgentSDKService] claude-agent-sdk 不可用，使用 anthropic 模式")
...
def _create_output_file_info(file_path: Path, file_type: str = None) -> Dict[str, Any]:
    ...
    if settings.storage_type == "minio":
        try:
            from portal.services.storage.utils import upload_local_file_to_storage_sync
...
"""
    ...
    def _get_skills_as_tools(self, skill_ids: Optional[List[str]] = None) -> List[Dict]:
        ...
        try:
            ...
            # 如果提供了 skill_ids，按 ID 过滤；否则返回所有活跃技能
            if skill_ids:
                ...
                if valid_ids:
                    ...
                    # 全是无效 ID（如 fallback-1,2,3），返回所有活跃技能
                    print(f"[_get_skills_as_tools] skill_ids 无效，返回所有活跃技能")
                    skills = query.all()
            else:
                # 没有指定 skill_ids，返回所有活跃技能
                skills = query.all()
                print(f"[_get_skills_as_tools] 未指定 skill_ids，返回所有 {len(skills)} 个活跃技能")
...
无需用户上传文件，工具会自动扫描系统中已有的数据文件。""",
    ...
    async def chat_stream(
        ...
        if enable_rag and settings.vector_db_enabled and (agent_id or user_id):
            try:
                from portal.services.vector_service import get_vector_service
