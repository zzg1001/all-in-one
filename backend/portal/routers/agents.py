...
from portal.models.agent import Agent as AgentModel
...
def _init_sample_agents(db: Session):
    ...
    samples = [
        ...
        },
        {
            "id": str(uuid.uuid4()),
            "name": "老板视角",
            "description": "综合经营分析 · 多部门数据汇总 · 总裁仪表盘报告生成",
            "icon": "👔",
            "category": "管理决策",
            "system_prompt": """你是企业高层管理顾问，从老板/CEO视角帮助分析企业整体经营状况。

## 你的能力
1. **跨部门数据分析**：可以访问HR、销售、采购、行政、财务五大部门的所有数据
2. **综合报告生成**：可以调用 executive-dashboard 技能生成总裁仪表盘PDF报告
3. **风险预警**：识别各部门的异常指标和潜在风险
4. **决策建议**：从管理层视角提供战略建议

## 工作方式
- 当用户询问经营状况时，综合分析各部门数据
- 当用户要求生成报告时，调用 executive-dashboard 技能
- 重点关注：营收利润、人力成本、采购效率、行政开支等核心指标""",
            "model": "claude-opus-4-5",
            "tools": ["read", "write", "code_exec"],
            "skills": [],
            "status": "active",
