...
def main(params: dict = None):
    ...
    if not excel_file:
        # 尝试多个可能的示例文件名
        sample_names = ["HR数据_示例.xlsx", "sample_hr_data.xlsx", "sample.xlsx"]
        for name in sample_names:
            sample_file = ASSETS_DIR / name
            if sample_file.exists():
                log(f"[hr-report] 未提供 Excel 文件，使用示例数据: {sample_file}")
                excel_file = str(sample_file)
                break
        else:
            # 没有找到示例文件，使用 --generate-sample 参数
            log("[hr-report] 未提供 Excel 文件，将生成示例数据")
            use_sample = True

    if excel_file:
        log(f"[hr-report] 输入文件: {excel_file}")

    # 调用报告生成脚本
    generator_script = SCRIPTS_DIR / "generate_hr_report.py"
    ...
    # 生成报告参数
    report_title = "人力资源分析报告"

    log(f"[hr-report] 运行脚本: {generator_script}")
    log(f"[hr-report] 输出目录: {OUTPUTS_DIR}")
    ...
    try:
        ...
        if use_sample:
            ...
            cmd.extend(["--input", str(excel_file)])
        ...
        log(f"[hr-report] returncode: {result.returncode}")
        if stdout:
            log(f"[hr-report] stdout: {stdout[:500]}")
        if stderr:
            log(f"[hr-report] stderr: {stderr[:500]}")
        ...
        for f in OUTPUTS_DIR.glob("*.pdf"):
            if time.time() - f.stat().st_mtime < 120:  # 最近2分钟生成的
                if "人力资源" in f.name or "HR" in f.name.upper():
                    pdf_file = f
                    break

        # 如果没找到特定名称的，取最新的 PDF
        if not pdf_file:
            pdf_files = sorted(OUTPUTS_DIR.glob("*.pdf"), key=lambda x: x.stat().st_mtime, reverse=True)
            for f in pdf_files:
        ...
        if pdf_file:
            return {
                "success": True,
                "output": f"人力资源分析报告已生成: {pdf_file.name}",
                "result": {
                    "input_file": str(excel_file) if excel_file else "示例数据",
                    "output_file": str(pdf_file)
                },
                "_output_file": {
                    "name": pdf_file.name,
                    "type": "pdf",
                    "url": f"/outputs/{pdf_file.name}",
                    "path": str(pdf_file)
                }
            }
        else:
